...
class PersonItemAdapter(private val onClickPerson:(Int) -> Unit) : ListAdapter<PersonItem, PersonItemViewHolder>(BaseDiffUtil()) {
    ...
    override fun onBindViewHolder(holder: PersonItemViewHolder, position: Int) {
        holder.bind(getItem(position), onClickPerson)
    }
}
